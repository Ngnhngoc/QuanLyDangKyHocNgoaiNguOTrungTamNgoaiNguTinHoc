...
export default function AdminNgonNgu() {
  ...
  const fetchNgonNgu = async () => {
    try {
      const res = await fetch('http://localhost:5052/api/DanhMucNN');
      if (res.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEditing 
      ? `http://localhost:5052/api/DanhMucNN/${maNn}` 
      : 'http://localhost:5052/api/DanhMucNN';
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5052/api/DanhMucNN/${id}`, { method: 'DELETE' });
      if (res.ok) {
