...
export default function AdminLopHoc() {
  ...
  const handleOpenStudentList = async (lophoc) => {
    ...
    setLoadingHV(true);
    try {
      const res = await fetch(apiUrl(`/LopHoc/${lophoc.maLop}/HocVien`));
      if (res.ok) {
  ...
  const handleRemoveStudent = async (maHv) => {
    ...
    try {
      const res = await fetch(apiUrl(`/LopHoc/${selectedLop.maLop}/HocVien/${maHv}`), {
        method: 'DELETE'
      });
      if (res.ok) {
        alert("Đã xóa học viên khỏi lớp thành công!");
        // Gọi lại API load danh sách học viên để làm mới bảng Modal
        const reloadRes = await fetch(apiUrl(`/LopHoc/${selectedLop.maLop}/HocVien`));
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(apiUrl(`/LopHoc/${id}`), { method: 'DELETE' });
      if (res.ok) {
